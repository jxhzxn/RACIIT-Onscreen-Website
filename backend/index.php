...
if(empty($name) or empty($age) or empty($email) or empty($contact) or empty($theme)){
    ...
    $sql = 'INSERT INTO competitors(cname,cage,cemail,ccontact,ctheme,cnic) VALUES(:cname,:cage,:cemail,:ccontact,:ctheme,:cnic)';
    $stmt = $con->prepare($sql);
    $stmt->execute(['cname'=>$name,'cage'=>$age,'cemail'=>$email,'ccontact'=>$contact,'ctheme'=>$theme,'cnic'=>$nic]);
